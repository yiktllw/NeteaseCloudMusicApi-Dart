...
void main() async {
  ...
  try {
    // 1. 搜索歌曲
    print('🎵 搜索 "周杰伦" 的歌曲...');
    var searchResult = await api
        .call(ApiModules.search, {'keywords': '周杰伦', 'type': 1, 'limit': 3});
    ...
    if (responseBody != null && responseBody['code'] == 200) {
      ...
      if (songs != null && songs.isNotEmpty) {
        ...
        for (var i = 0; i < songs.length; i++) {
          ...
          var artistName = artists != null && artists.isNotEmpty
              ? artists[0]['name']
              : '未知歌手';
        ...
        print('\n🎤 获取歌曲详情...');
        var songDetailResult = await api.call(
            ApiModules.songDetail, ApiParams.songDetail(ids: firstSongId));
        ...
        if (responseBody != null && responseBody['code'] == 200) {
          ...
          if (songDetails != null && songDetails.isNotEmpty) {
            ...
            print(
                '   时长: ${Duration(milliseconds: songDetail['dt']).inMinutes}:${(Duration(milliseconds: songDetail['dt']).inSeconds % 60).toString().padLeft(2, '0')}');
          }
        }
    ...
    print('\n📻 获取个性化推荐...');
    var recommendResult =
        await api.call(ApiModules.personalized, {'limit': 5, 'cookie': cookie});
    ...
    if (responseBody != null && responseBody['code'] == 200) {
      ...
      if (playlists != null && playlists.isNotEmpty) {
        ...
        for (var i = 0; i < playlists.length; i++) {
          ...
          print(
              '   ${i + 1}. ${playlist['name']} (播放量: ${playlist['playCount']})');
        }
      }
    }
    ...
    // 5. 获取用户详情示例
    print('\n👤 获取用户详情...');
    var userResult = await api.api.userDetail(
        uid: "375334328",
