...
Future<Map<String, dynamic>> userDetail(
  ...
  final response = await request('/api/v1/user/detail/${query['uid']}', data, RequestOptions.create(query, crypto: 'weapi'));
  
  // 参考原项目，处理 avatarImgId_str 字段
  final responseStr = jsonEncode(response);
  final processedStr = responseStr.replaceAll('avatarImgId_str', 'avatarImgIdStr');
