...
class ApiCache {
  ...
  /// 判断是否应该缓存响应
  bool _shouldCacheResponse(Map<String, dynamic> request, Map<String, dynamic> response, bool Function(Map<String, dynamic>, Map<String, dynamic>)? toggle) {
  ...
  /// 创建缓存对象
  Map<String, dynamic> _createCacheObject(int status, Map<String, String> headers, dynamic data) {
    return {
      'status': status,
      'headers': headers,
      'data': data,
  ...
  Future<Map<String, dynamic>> middleware(
    ...
    final durationMs = _parseDuration(duration, _globalOptions['defaultDuration'] as int);
  ...
  void clear([String? target, bool isAutomatic = false]) {
    ...
    _debug('clearing ${isAutomatic ? "expired" : "cached"} entry for "$target"');
